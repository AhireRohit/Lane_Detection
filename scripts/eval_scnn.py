...
from src.scnn import SCNN

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", type=Path, default=CHECKPOINTS_DIR / "scnn_best.pt")
    ...
    model = SCNN(base=base).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    totals = {"iou": 0.0, "f1": 0.0, "precision": 0.0, "recall": 0.0}
    latencies = []
    vis_dir = OUTPUTS_DIR / "scnn_preds"
    ...
    out_json = OUTPUTS_DIR / "scnn_eval.json"
