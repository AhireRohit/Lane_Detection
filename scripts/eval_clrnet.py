...
def main():
    ...
    parser.add_argument("--checkpoint", type=Path, default=CHECKPOINTS_DIR / "clrnet_best.pt")
    ...
    model = CLRNetSeg(base=base).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    totals = {"iou": 0.0, "f1": 0.0, "precision": 0.0, "recall": 0.0}
    latencies = []
    vis_dir = OUTPUTS_DIR / "clrnet_preds"
    ...
    out_json = OUTPUTS_DIR / "clrnet_eval.json"
