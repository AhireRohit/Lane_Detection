...
def main():
    ...
    parser.add_argument("--checkpoint", type=Path, default=CHECKPOINTS_DIR / "laneatt_best.pt")
    ...
    model = LaneATTSeg(base=base).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    totals = {"iou": 0.0, "f1": 0.0, "precision": 0.0, "recall": 0.0}
    latencies = []
    vis_dir = OUTPUTS_DIR / "laneatt_preds"
    ...
    out_json = OUTPUTS_DIR / "laneatt_eval.json"
